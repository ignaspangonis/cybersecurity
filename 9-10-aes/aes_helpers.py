...
# konvertuoja šifrą į blokus. Naudoti raktui, šifrui ir IV
def byte_string_to_blocks(string):
        return bytes_to_blocks(bytes.fromhex(string))

# konvertuoja blokus į šifrą
...
def inverse_shift_rows(state):
    """
    Perform the Inverse Shift Rows operation on the AES state matrix.
    The state should be a 4x4 list of bytes.
    """
    # Row 0: No shift
...
def shift_rows(state):
    """
    Perform the ShiftRows step of AES.

    Args:
        state (list of list of int): A 4x4 matrix representing the state.

    Returns:
        list of list of int: The transformed state after ShiftRows.
    """
...
# Galois field multiplication for AES inverse MixColumns
def gmul(a, b):
    """Galois Field (GF(2^8)) multiplication of a and b."""
    p = 0
    for i in range(8):
        if b & 1:
            p ^= a
...
def inverse_mix_columns(state):
    """
    Perform the Inverse MixColumns operation on the AES state.

    :param state: A 4x4 list of bytes (integers 0-255) representing the AES state.
    :return: A 4x4 list of bytes after the Inverse MixColumns transformation.
    """
    ...
    for c in range(4):
        new_state[0][c] = (
            mult_e[state[0][c]] ^ mult_b[state[1][c]] ^
            mult_d[state[2][c]] ^ mult_9[state[3][c]]
        )
        new_state[1][c] = (
            mult_9[state[0][c]] ^ mult_e[state[1][c]] ^
            mult_b[state[2][c]] ^ mult_d[state[3][c]]
        )
        new_state[2][c] = (
            mult_d[state[0][c]] ^ mult_9[state[1][c]] ^
            mult_e[state[2][c]] ^ mult_b[state[3][c]]
        )
        new_state[3][c] = (
            mult_b[state[0][c]] ^ mult_d[state[1][c]] ^
            mult_9[state[2][c]] ^ mult_e[state[3][c]]
        )
...
def mix_columns(state):
    ...
    :param state: A 4x4 list of bytes (integers 0-255) representing the AES state.
    :return: A 4x4 list of bytes after the MixColumns transformation.
    """
    ...
    for c in range(4):
        new_state[0][c] = (
            mult_2[state[0][c]] ^ mult_3[state[1][c]] ^ state[2][c] ^ state[3][c]
        )
        new_state[1][c] = (
            state[0][c] ^ mult_2[state[1][c]] ^ mult_3[state[2][c]] ^ state[3][c]
        )
        new_state[2][c] = (
            state[0][c] ^ state[1][c] ^ mult_2[state[2][c]] ^ mult_3[state[3][c]]
        )
        new_state[3][c] = (
            mult_3[state[0][c]] ^ state[1][c] ^ state[2][c] ^ mult_2[state[3][c]]
        )
